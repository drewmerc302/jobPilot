...
# Dedupe unknown-model warnings — without this, every LLM call for an
# uncatalogued model logs a line, which can be dozens per refresh and
# bury real signal in the log file.
...
def estimate_cost(model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    if rates is None:
        if model not in _warned_unknown:
            ...
            logger.warning(
                "Unknown model %r in pricing table; defaulting to sonnet-class "
                "rates. Cost meter will be inaccurate until this is added.",
                model,
            )
